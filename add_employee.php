<?php
require_once 'db.php';
require_once 'TeamManager.php'; // Include our new class
session_start();
...
if ($_POST) {
    ...
    if (!empty($name) && !empty($start_date) && !empty($skills)) {
        try {
            ...
            $stmt->execute([
                $name, json_encode($skills), $hourly_rate, $pay_period, $max_hours, $trial_months,
                $collective_agreement, $insurance, $start_date, $end_date, $principal_duties
            ]);

            // --- THIS IS THE NEW AUTOMATION LOGIC ---
            // After successfully adding the employee, check if a new team can be formed.
            $teamManager = new TeamManager($db);
            $automationMessages = $teamManager->autoGroupUngroupedEmployees();
            
            // Start with the basic success message
            ...
            // If any teams were created, append those messages
            if (!empty($automationMessages)) {
                foreach($automationMessages as $autoMessage) {
                    $message .= '<div class="alert alert-info mt-2"><i class="fas fa-users-cog"></i> ' . $autoMessage . '</div>';
                }
            }
            // --- END OF NEW LOGIC ---
