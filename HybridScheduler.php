...
class HybridScheduler {
    ...
    public function runOptimizationForTasks($taskIds) {
        $startTime = microtime(true);
        try {
            // Phase 1: Rule-Based Filtering for the specific tasks
            $phase1Results = $this->phaseOneForSpecificTasks($taskIds);

            if (empty($phase1Results['filtered_assignments'])) {
                return [
                    'success' => false,
                    'message' => 'No valid assignments found for the provided tasks after rule-based filtering',
                    'phase1_results' => $phase1Results,
                    'assignments' => [],
                    'statistics' => [],
                    'execution_time' => microtime(true) - $startTime
    ...
    private function phaseOneRuleBasedFiltering() {
        ...
        if (empty($unassignedTasks)) {
            return [
                'filtered_assignments' => [], 'total_tasks' => 0, 'total_teams' => count($activeTeams),
                'filtering_stats' => ['tasks_processed' => 0, 'valid_assignments_found' => 0, 'average_options_per_task' => 0]
            ];
        }
    ...
    private function phaseOneForSpecificTasks($taskIds) {
        // Get specific unassigned tasks and active teams
        $tasksToProcess = $this->ruleBasedAlgorithm->getTasksByIds($taskIds);
        $activeTeams = $this->ruleBasedAlgorithm->getActiveTeams();
        ...
        // Apply rule-based filtering
        $filteredAssignments = $this->ruleBasedAlgorithm->filterAssignments($tasksToProcess, $activeTeams);
        ...
        $averageOptionsPerTask = count($tasksToProcess) > 0 ? $totalOptions / count($tasksToProcess) : 0;
        
        return [
            'filtered_assignments' => $filteredAssignments,
            'total_tasks' => count($tasksToProcess),
            'total_teams' => count($activeTeams),
            'filtering_stats' => [
                'tasks_processed' => count($tasksToProcess),
                'valid_assignments_found' => $validAssignmentsFound,
                'total_assignment_options' => $totalOptions,
                'average_options_per_task' => round($averageOptionsPerTask, 2)
            ]
    ...
    private function generateDebugInfo($phase1Results, $phase2Results, $applicationResults) {
            ...
            foreach ($phase1Results['filtered_assignments'] as $taskId => $assignmentData) {
                ...
                $debugInfo['rule_based_filtering'][] = [
                    ...
                    'feasible_teams' => array_map(function($teamData) {
                        return [
                            'team_name' => (isset($teamData['team']) && isset($teamData['team']['team_name'])) ? $teamData['team']['team_name'] : '-',
                            'suitability_score' => isset($teamData['suitability_score']) ? $teamData['suitability_score'] : '-',
                            'skill_match' => isset($teamData['skills_match']) ? $teamData['skills_match'] : '-',
                            'availability_score' => isset($teamData['availability_score']) ? $teamData['availability_score'] : '-'
                        ];
        ...
        $finalAssignments = $phase2Results['optimized_assignments'] ?? [];
        if (!empty($finalAssignments)) {
        ...
        // --- NEW LOGIC: Find unassigned tasks and determine the REAL reason ---
        $assignedTaskIds = array_column($finalAssignments, 'task_id');
        $allTaskIds = array_keys($phase1Results['filtered_assignments'] ?? []);
        $unassignedTaskIds = array_diff($allTaskIds, $assignedTaskIds);

        // Create a lookup for which teams are busy at which time slots
        $teamSlotUsage = [];
        foreach ($finalAssignments as $assignment) {
            $key = $assignment['team_id'] . '_' . $assignment['task']['scheduled_date'] . '_' . $assignment['task']['time_slot'];
            $teamSlotUsage[$key] = $assignment['task_id'];
        }
        ...
        foreach ($unassignedTaskIds as $taskId) {
            ...
            if (empty($assignmentData['valid_teams'])) {
                ...
                // Check for the new, specific reason: resource conflict
                $areAllTeamsBusy = true;
                ...
                foreach($validTeamIds as $teamId) {
                    ...
                    if (!isset($teamSlotUsage[$slotKey])) {
                        $areAllTeamsBusy = false; // Found at least one valid team that was not busy.
                        break;
                    }
                }

                if ($areAllTeamsBusy) {
                    $reason = 'Resource Conflict: All qualified teams are already booked in this time slot.';
                } else {
                    $reason = 'Could not find optimal assignment (may conflict with higher priority tasks or workload balance rules).';
                }
            }
