    ...
    if (!empty($client) && !empty($service_type) && !empty($scheduled_date) && !empty($time_slot) && !empty($selected_locations)) {
        try {
            ...
            foreach ($selected_locations as $location) {
                ...
                $stmt->execute([
                    $task_title,
                    json_encode($service_info['skills']),
                ...
                $tasks_created++;
                $new_task_ids[] = $db->lastInsertId();
            }

            $db->commit();

            // --- HybridScheduler assignment ---
            ...
            if ($result['success'] && !empty($result['assignments'])) {
                $assignment_html .= '<div class="alert alert-info mt-3"><strong>Task Assignments:</strong><ul>';
                foreach ($result['assignments'] as $assignment) {
                    // Only show assignments for newly created tasks
                    if (in_array($assignment['task_id'], $new_task_ids)) {
                        $team_name = $assignment['team']['team_name'] ?? 'Unassigned';
                        $assignment_html .= '<li><b>' . htmlspecialchars($assignment['task']['title']) . '</b> &rarr; <span class="badge bg-primary">' . htmlspecialchars($team_name) . '</span></li>';
                    }
                }
                $assignment_html .= '</ul></div>';
            } else {
                $assignment_html .= '<div class="alert alert-warning mt-3">No assignments could be made for the new tasks.</div>';
            }
