...
if ($_POST && isset($_POST['run_optimization'])) {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        error_log('CSRF token mismatch in optimization request');
    }
    // Authorization check
    elseif (!isAuthorizedUser()) {
        $optimization_message = '<div class="alert alert-danger"><i class="fas fa-exclamation-triangle"></i> Access denied: You do not have permission to run optimization.</div>';
        error_log('Unauthorized optimization attempt');
    }
    else {
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <small>
                                            <?php 
                                            $skills = json_decode($task['required_skills'], true);
                                            echo is_array($skills) ? implode(', ', array_slice($skills, 0, 2)) : 'None';
                                            if (is_array($skills) && count($skills) > 2) echo '...';
                                            ?>
                                            </small>
                                        </td>
                                        <td>
    ...
    <script>
        ...
        function filterTasks() {
            ...
            rows.forEach(row => {
                ...
                // Date filter
                if (dateFilter && date !== 'Not scheduled') {
                    const rowDate = new Date(row.cells[4].dataset.date || dateCell.textContent);
