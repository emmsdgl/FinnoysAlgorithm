...
class GeneticAlgorithm {
        ...
        for ($i = 0; $i < $this->populationSize; $i++) {
            ...
            foreach ($filteredAssignments as $taskId => $assignmentData) {
                if (!empty($assignmentData['valid_teams'])) {
                    ...
                    if (
                        $selectedTeam &&
                        is_array($selectedTeam) &&
                        isset($selectedTeam['team']) &&
                        isset($selectedTeam['team']['id'])
                    ) {
    ...
    private function evaluateFitness($individual, $filteredAssignments) {
        // --- CHANGE IS HERE: Comment out the overlap penalty check ---
    ...
    private function calculatePrioritySatisfaction($individual, $filteredAssignments) {
        ...
        foreach ($individual as $taskId => $assignedTeamId) {
            ...
            foreach ($assignmentData['valid_teams'] as $teamData) {
                if (
                    isset($teamData['team']) &&
                    isset($teamData['team']['id']) &&
                    $teamData['team']['id'] == $assignedTeamId
                ) {
    ...
    private function crossover($parent1, $parent2, $filteredAssignments) {
        ...
        // --- THIS IS THE FIX ---
        // A crossover operation is not possible or meaningful with only one task (or none).
        // If we don't have at least two tasks, we can't pick a point between them to swap.
        if (count($taskIds) <= 1) {
            return [$parent1, $parent2]; // Return the parents unmodified to prevent an error.
        }
        // --- END OF FIX ---

        // This line is now safe because count($taskIds) is guaranteed to be 2 or more.
    ...
    private function ensureValidAssignment($individual, $filteredAssignments) {
        foreach ($individual as $taskId => $teamId) {
            ...
            $validTeamIds = [];
            foreach ($assignmentData['valid_teams'] as $teamData) {
                if (isset($teamData['team']) && isset($teamData['team']['id'])) {
                    $validTeamIds[] = $teamData['team']['id'];
                }
            }
            if (!in_array($teamId, $validTeamIds)) {
                ...
                foreach ($assignmentData['valid_teams'] as $teamData) {
                    if (isset($teamData['team']) && isset($teamData['team']['id'])) {
                        $individual[$taskId] = $teamData['team']['id'];
                        break;
                    }
                }
            }
    ...
    private function convertToAssignments($solution, $filteredAssignments) {
        $assignments = [];
        // --- REMOVED: The $teamSlotUsage tracker is no longer needed.
        // $teamSlotUsage = []; 

        // 1. Initialize workload trackers for this optimization run
        $teamWorkloadMinutes = [];
        $newlyAssignedTaskCount = []; // For the "N+1" balance rule
        $teams = $this->ruleBasedAlgorithm->getActiveTeams();
        foreach ($teams as $team) {
            $teamWorkloadMinutes[$team['id']] = $this->getCurrentTeamWorkload($team['id']);
            $newlyAssignedTaskCount[$team['id']] = 0;
        }
        ...
        usort($sortedTaskIds, function($a, $b) use ($filteredAssignments) {
            ...
            $priorityComparison = $priorityOrder[$taskA['priority_level']] <=> $priorityOrder[$taskB['priority_level']];
            if ($priorityComparison !== 0) {
                return $priorityComparison;
            }
            return strtotime($taskA['created_at']) <=> strtotime($taskB['created_at']);
        });

        // 3. Iteratively assign tasks according to the new rules
        foreach ($sortedTaskIds as $taskId) {
            $taskData = $filteredAssignments[$taskId];
            $task = $taskData['task'];
            $validTeamsForTask = $taskData['valid_teams'];

            if (empty($validTeamsForTask)) {
                continue;
            }

            $assignedTeamId = null;
            $bestOptionFound = false;

            $potentialTeams = [];
            $gaSuggestedTeamId = $solution[$taskId] ?? null;

            if ($gaSuggestedTeamId) {
                foreach ($validTeamsForTask as $validTeamData) {
                    if ($validTeamData['team']['id'] == $gaSuggestedTeamId) {
                        $potentialTeams[] = $validTeamData;
                        break;
                    }
                }
            }
            ...
            $minTasksAcrossTeams = empty($newlyAssignedTaskCount) ? 0 : min($newlyAssignedTaskCount);

            foreach ($potentialTeams as $teamData) {
                $teamId = $teamData['team']['id'];
                
                // --- REMOVED: The strict time slot check. ---
                // $slotKey = $teamId . '_' . $task['scheduled_date'] . '_' . $task['time_slot'];
                // if (isset($teamSlotUsage[$slotKey])) {
                //     continue; 
                // }

                if (($newlyAssignedTaskCount[$teamId] + 1) > ($minTasksAcrossTeams + 1)) {
                    continue;
                }
            ...
            if (!$bestOptionFound && in_array($task['priority_level'], ['urgent', 'high'])) {
                foreach ($potentialTeams as $teamData) {
                    $teamId = $teamData['team']['id'];
                    // --- REMOVED: Time slot check from the fallback as well ---
                    // $slotKey = $teamId . '_' . $task['scheduled_date'] . '_' . $task['time_slot'];
                    // if (!isset($teamSlotUsage[$slotKey])) {
                    $assignedTeamId = $teamId;
                    break;
            ...
            if (!$assignedTeamId) {
                $leastBusyTeamId = null;
                $validTeamIds = array_column(array_column($validTeamsForTask, 'team'), 'id');
                
                $sortedValidTeams = [];
                foreach ($validTeamIds as $validTeamId) {
                    $sortedValidTeams[$validTeamId] = $newlyAssignedTaskCount[$validTeamId];
                }
                asort($sortedValidTeams);

                foreach (array_keys($sortedValidTeams) as $teamId) {
                    // --- REMOVED: Final time slot check ---
                    // $slotKey = $teamId . '_' . $task['scheduled_date'] . '_' . $task['time_slot'];
                    // if (!isset($teamSlotUsage[$slotKey])) {
                    $leastBusyTeamId = $teamId;
                    break;
                    // }
                }
                $assignedTeamId = $leastBusyTeamId;
            }

            if ($assignedTeamId) {
                // --- REMOVED: No longer need to mark the slot as used ---
                // $slotKey = $assignedTeamId . '_' . $task['scheduled_date'] . '_' . $task['time_slot'];
                // $teamSlotUsage[$slotKey] = $taskId;

                $teamWorkloadMinutes[$assignedTeamId] += $task['estimated_duration'];
                $newlyAssignedTaskCount[$assignedTeamId]++;

                $assignments[] = [
                    'task_id' => $taskId,
                    'team_id' => $assignedTeamId,
                    'task' => $task,
                    'team' => $this->getTeamById($assignedTeamId),
